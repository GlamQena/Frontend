...
export const logout = async () => {
  try {
    ...
    const response = await fetch(
      `${API_URL}/logout${session_id ? `?session_id=${session_id}` : ""}`,
      { method: "DELETE" },
    );
...
export const getAccessToken = async (setFormMessage) => {
  try {
    ...
    if (!accessToken) {
      responseMessageSetter(false, "please login first", setFormMessage);
      return null;
    }
    ...
    if (decodedAccessToken.exp * 1000 < Date.now()) {
      ...
      if (refreshData.message.includes("expired")) {
        ...
        responseMessageSetter(false, refreshData.message, setFormMessage);
        return null;
      } else {
...
export function responseMessageSetter(success, message, setFormMessage) {
