...
const Profile = () => {
  ...
  const handleChangeInput = (e) => {
    ...
    if (type === "password")
      ...
      else newNotifications.push(name);
      ...
      setProfileForm((prev) => ({
        ...prev,
        store_address: { ...prev.store_address, [name]: value },
      }));
    ...
    } else setProfileForm((prev) => ({ ...prev, [name]: value }));
  };

  const handleSelection = (field, value) => {
    if (typeof profileForm[field] === "object") {
      //array selection like skinConcerns
    ...
    } else setProfileForm((prev) => ({ ...prev, [field]: value }));
  };
  ...
  const handleChangePassword = async (e) => {
    ...
    if (
      ...
      responseMessageSetter(false, "كلمة المرور غير متطابقة", setFormMessage);
      return;
    }
  ...
  const handleEditProfile = async (e) => {
      ...
      try {
        ...
        if (!response.ok)
          return responseMessageSetter(
            false,
            avatarResData.message,
            setFormMessage,
          );

        responseMessageSetter(true, avatarResData.message, setFormMessage);
      } catch (error) {
        responseMessageSetter(false, error.message, setFormMessage);
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        responseMessageSetter(
          true,
          data.message || "تم حفظ التعديلات بنجاح ",
          setFormMessage,
        );
        localStorage.setItem("user", JSON.stringify(data.user));
        setEditMode(false);
  ...
  const handleDeleteProfile = async () => {
    try {
      ...
      } else responseMessageSetter(false, data.message, setFormMessage);
    } catch (err) {
  ...
  return (
    <div className="profile-page-wrapper" dir="rtl">
        ...
        <section className="profile-header-card">
          <div className="user-main-info">
            ...
            <div className="user-text">
              ...
              {/*+ has more precedence than || so it needs association*/}
              ...
              {((profileForm.gender && !editMode) || editMode) && (
                <div className="gender-toggle">
                  ...
                  <button
                    ...
                    style={{
                      opacity:
                        !editMode && profileForm.gender !== "female" ? 0.5 : 1,
                    }}
                  >
                    أنثى
                  </button>
                  <button
                    ...
                    style={{
                      opacity:
                        !editMode && profileForm.gender !== "male" ? 0.5 : 1,
                    }}
                    disabled={!editMode}
              ...
              {((profileForm.address && !editMode) || editMode) && (
                <div className="address-fields">
                  ...
                  <div className="address-inputs">
                    ...
                    {((profileForm.address?.district && !editMode) ||
                      editMode) && (
                      <div className="input-field">
                        <label>المنطقة</label>
                        <input
                    ...
                    {((profileForm.address?.street && !editMode) ||
                      editMode) && (
                      <div className="input-field">
                        <label>الشارع</label>
                        <input
                    ...
                    {["جافة", "دهنية", "مختلطة", "حساسة", "عادية"].map(
                      (type) => (
                        <button
                          ...
                          style={{
                            opacity:
                              !editMode && profileForm.skinType !== type
                                ? 0.5
                                : 1,
                          }}
                          disabled={!editMode}
            ...
            {profileForm.role === "store_owner" && (
              <React.Fragment>
                ...
                <section className="form-card">
                  ...
                  {((profileForm.store_description && !editMode) ||
                    editMode) && (
                    <textarea
                ...
                {((profileForm.bankAccount && !editMode) || editMode) && (
                  <section className="form-card">
                    ...
                    {((profileForm.bankAccount?.bankName && !editMode) ||
                      editMode) && (
                      <div className="input-field">
                        <label>اسم البنك</label>
                        <input
                          name="bankName"
                          value={profileForm.bankAccount?.bankName}
                          onChange={handleChangeInput}
                          readOnly={!editMode}
                        />
                      </div>
                    ...
                    {((profileForm.bankAccount?.accountNumber && !editMode) ||
                      editMode) && (
                      <div className="input-field">
                    ...
                    {((profileForm.bankAccount?.accountName && !editMode) ||
                      editMode) && (
                      <div className="input-field">
                        <label>اسم الحساب</label>
                        <input
                          name="accountName"
                          value={profileForm.bankAccount?.accountName}
                          onChange={handleChangeInput}
                          readOnly={!editMode}
                        />
                      </div>
            ...
            <section className="form-card">
              ...
              <label className="checkbox-item">
                <input
                  type="checkbox"
                  name="email"
                  checked={profileForm.notifications?.includes("email")}
                  onChange={handleChangeInput}
                  disabled={!editMode}
                />
                البريد الإلكتروني
              ...
              <label className="checkbox-item">
                <input
                  type="checkbox"
                  name="push"
                  checked={profileForm.notifications?.includes("push")}
                  onChange={handleChangeInput}
                  disabled={!editMode}
                />
                تنبيهات التطبيق
              ...
              <label className="checkbox-item">
                <input
                  type="checkbox"
                  name="sms"
                  checked={profileForm.notifications?.includes("sms")}
                  onChange={handleChangeInput}
                  disabled={!editMode}
                />
                الرسائل النصية SMS
      ...
      <form
        className="password-form form-card"
