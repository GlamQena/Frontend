    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-stars-row" onMouseLeave={() => setHovered(0)}>
          {[1, 2, 3, 4, 5].map((n) => (
            <span key={n} onMouseEnter={() => setHovered(n)}>
              <RatingStar filled={(hovered || rating) >= n} onClick={() => setRating(n)} />
            </span>
          ))}
        </div>
        {rating > 0 && (
        ...
        <div className="modal-actions">
          ...
          <button className="modal-submit" onClick={submit} disabled={!rating || loading}>
...
function ProductsList({ 
  ...
  if (isStoreOwner) {
    allStores = [{
      owner_store_id: order.owner_store_id || order.store_id || order.store?.owner_store_id || null,
      products: Array.isArray(order.store_products) ? order.store_products : [],
  ...
  allStores.forEach((store) => {
    const storeName = store.owner_store_id?.store_name || order.store_name || "—";
    ...
    <div className="od-products-card">
      ...
      <div className="od-products-list">
        {paginatedProducts.map((prod, prodIdx) => {
          const productData = prod?.prod_id && typeof prod.prod_id === "object" ? prod.prod_id : null;
          const realProdId = productData?._id || prod?.prod_id || prod?.product_id || prod?._id;
          const productName = prod?.name || productData?.name || "—";
          const imgSrc = buildImgSrc(productData?.images?.[0] || prod?.images?.[0]);
          const quantity = Number(prod?.quantity) || 0;
          const subtotalPrice = Number(prod?.subtotal_price ?? prod?.subtotal ?? 0);
          const hasReviewed = prod?.hasReviewed === true || ratedProducts.includes(realProdId?.toString());
            ...
            >
              ...
              <div className="od-product-left">
                ...
                <div className="od-product-info">
                  ...
                  {!isStoreOwner && <p className="od-prod-store">من: {prod.storeName}</p>}
                </div>
              </div>
              ...
              <div className="od-product-right">
                ...
                <span className="od-prod-price">{formatCurrency(subtotalPrice)}</span>
                ...
                {!isAdmin && !isStoreOwner && isDelivered && showReviewBtn && (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!hasReviewed) setReview?.({ prod, storeOwnerId: prod.storeId });
                    }}
...
// ─── Client Order View ──────────────────────────────────────────────────────
// ─── Client Order View ──────────────────────────────────────────────────────
function ClientOrderView({ order, normalizedStatus, isCancelled, ratedProducts, setReview }) {
  ...
  const payBadge = PAYMENT_STATUS_CONFIG[resolvedPaymentStatus()] || PAYMENT_STATUS_CONFIG.pending;
      ...
      <aside className="od-tracking-card">
        ...
        <div className="od-steps">
          {ORDER_STEPS.filter((step) => {
            ...
            return (
              <div key={step.key} className={`od-step ${isDone ? "od-step--done" : ""} ${isPend ? "od-step--pending" : ""} ${isLast ? "od-step--last" : ""}`}>
                ...
                <div className="od-step-text">
                  ...
                  <span className="od-step-time">{isDone ? formatDate(order.updatedAt) : "-- : --"}</span>
                </div>
              </div>
            );
          })}
          {isCancelled && (
            <div className="od-step od-step--cancelled od-step--last">
              ...
              <div className="od-step-text">
                ...
                <span className="od-step-time cancelled-time">{formatDate(order.cancelledAt || order.updatedAt)}</span>
              </div>
            </div>
          )}
        </div>
        <div className="od-totals">
          <div className="od-total-row">
            ...
            <span className="od-total-val">{formatCurrency(order.subtotal_price)}</span>
          </div>
          <div className="od-total-row">
            <span className="od-total-label">الشحن</span>
            <span className="od-total-val">{formatCurrency(order.delivery_cost)}</span>
          </div>
          <div className="od-total-row od-grand-row">
            <span className="od-grand-label">الإجمالي</span>
            <span className="od-grand-val">{formatCurrency(order.total_price)}</span>
          </div>
        </div>
      </aside>
      ...
      <div className="od-card od-payment-card">
        ...
        <div className="od-info-list">
          <div className="od-info-row">
            ...
            <span className="od-info-val od-method-val">{PAYMENT_METHOD_MAP[order.payment?.method] || order.payment?.method}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">حالة الدفع</span>
            <span className={`od-pay-badge ${payBadge.cls}`}>{payBadge.label}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">تاريخ العملية</span>
            <span className="od-info-val">{formatDate(order.payment?.completedAt || order.createdAt)}</span>
          </div>
          {order.payment?.paymob_order_id && (
            <div className="od-info-row">
              <span className="od-info-label">رقم العملية</span>
              <span className="od-info-val od-mono">{order.payment.paymob_order_id}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="od-card od-shipping-card">
        ...
        <div className="od-info-list">
          <div className="od-info-row">
            ...
            <span className="od-info-val">{customerInfo?.firstName ? `${customerInfo.firstName} ${customerInfo.lastName || ""}`.trim() : "—"}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">الهاتف</span>
            <span className="od-info-val od-mono">{customerInfo?.phoneNumber || "—"}</span>
          </div>
          <div className="od-info-row od-address-row">
            ...
            <span className="od-info-val od-address-val">
              {customerInfo?.address ? `${customerInfo.address.street || ""}، ${customerInfo.address.city || ""}، مصر` : "—"}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="od-subtotals-section">
          ...
          <div className="od-subtotals-list">
            {allStores.map((store, idx) => {
              ...
              const percentage = totalOrderPrice > 0 ? (storeSubtotal / totalOrderPrice * 100).toFixed(1) : 0;
              ...
              return (
                <div key={idx} className="od-subtotal-item">
                  ...
                  <div className="od-subtotal-item-value-wrapper">
                    <span className="od-subtotal-item-value">{formatCurrency(storeSubtotal)}</span>
      ...
      <div className="od-card od-client-info-card">
        <div className="od-card-header">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" fill="var(--primary-main)"/>
          </svg>
      ...
      <div className="od-card od-order-info-card">
        ...
        <div className="od-info-list">
          <div className="od-info-row">
            ...
            <span className="od-info-val od-mono">#{order.order_id?.slice(-6).toUpperCase()}</span>
          </div>
          <div className="od-info-row">
          ...
          <div className="od-info-row">
            ...
            <span className="od-info-val">{order.store_products?.length || 0} منتج</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">الإجمالي:</span>
            <span className="od-info-val od-grand-highlight">{formatCurrency(order.store_subtotal)}</span>
          </div>
        </div>
      </div>
    ...
    <div className="od-grid od-grid--store">
      <div className="od-card od-client-info-card">
        ...
        <div className="od-client-name">
          {customer.firstName ? `${customer.firstName} ${customer.lastName || ""}`.trim() : customer.username || "—"}
        </div>
        <div className="od-info-list">
          <div className="od-info-row od-icon-row">
            <span className="od-icon-label">📞</span>
          ...
          <div className="od-info-row od-icon-row">
            <span className="od-icon-label">📍</span>
            <span className="od-info-val">{customer.address ? `${customer.address.street || ""}، ${customer.address.city || ""}، مصر` : "—"}</span>
          </div>
          <div className="od-info-row od-icon-row">
            <span className="od-icon-label">✉️</span>
            <span className="od-info-val">{customer.email || "—"}</span>
      ...
      <div className="od-card od-order-info-card">
        ...
        <div className="od-info-list">
          <div className="od-info-row">
            <span className="od-info-label">رقم الطلب:</span>
            <span className="od-info-val od-mono">#{order._id?.slice(-6).toUpperCase()}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">التاريخ:</span>
          ...
          <div className="od-info-row">
            <span className="od-info-label">المنتجات:</span>
            <span className="od-info-val">{order.products?.reduce((acc, s) => acc + (s.products?.length || 0), 0)} منتج</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">الإجمالي:</span>
            <span className="od-info-val od-grand-highlight">{formatCurrency(order.total_price)}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">طريقة الدفع:</span>
            <span className="od-info-val">{PAYMENT_METHOD_MAP[order.payment?.method] || "—"}</span>
          </div>
          <div className="od-info-row">
            <span className="od-info-label">حالة الدفع:</span>
...
export default function OrderDetailsList() {
  ...
  const statusConfig = STATUS_CONFIG[rawStatus] || STATUS_CONFIG["قيد الانتظار"];
  ...
  return (
    <div className="od-root" dir="rtl">
      <div
        ...
        onClick={() => navigate(storeMode ? "/dashboard/store_owner/orders" : "/orders")}
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M12.175 9H0V7H12.175L6.575 1.4L8 0L16 8L8 16L6.575 14.6L12.175 9Z" fill="var(--text-secondary)"/>
        </svg>
      ...
      <header className="od-header">
        <div className="od-header-info">
          <div className="od-title-row">
            <h1 className="od-order-number">رقم الطلب #{displayId?.slice(-6).toUpperCase()}</h1>
        ...
        {(clientMode || storeMode) && (
          <button className="od-print-btn" onClick={() => window.print()}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 01-2 2h-2M6 14h12v8H6z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            تحميل الفاتورة
          </button>
        )}
      ...
      {storeMode && (
        <StoreOrderView order={order} normalizedStatus={normalizedStatus} isCancelled={isCancelled} />
      )}

      {Adminmode && (
        <AdminOrderView order={order} normalizedStatus={normalizedStatus} isCancelled={isCancelled} />
      )}
      ...
      {review && clientMode && (
        <ReviewModal
          ...
          onSuccess={() => {
            const realProdId = review.prod?.prod_id && typeof review.prod.prod_id === "object"
            ...
            setOrder((prev) => ({
              ...
              products: prev.products.map((store) => ({
                ...
                products: store.products.map((p) => {
                  ...
                  return pId?.toString() === realProdId.toString() ? { ...p, hasReviewed: true } : p;
                }),
              })),
            }));
            setReview(null);
