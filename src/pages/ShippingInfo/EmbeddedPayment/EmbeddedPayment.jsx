...
const EmbeddedPayment = ({
    ...
    try {
      ...
      const response = await checkoutPayment(
        ...
        JSON.stringify({
          ...
          redirect_url: process.env.REACT_APP_ENVIRONMENT === "production" ? 
            "https://frontend-pearl-delta-37.vercel.app/orders" :
      ...
      if (selectedMethod === "cash") {
        ...
        responseMessageSetter(
          true,
          data.message || "تم تأكيد الطلب بنجاح!",
          setActionMsg,
        );
        ...
        if (data.redirect_url) {
          ...
          responseMessageSetter(
            true,
            "🔄 جاري توجيهك إلى صفحة الدفع بالمحفظة...",
            setActionMsg,
          );
      ...
      if (error?.code === "AUTH_EXPIRED" || msg.includes("session")) {
        responseMessageSetter(
          false,
      ...
      // Everything else *is* a real payment failure.
      responseMessageSetter(
        false,
        error.message || "حدث خطأ في الدفع",
        setActionMsg,
      );
      setPaymentStatus("failed");
      setLoading(false);
  ...
  useEffect(() => {
    ...
    if (walletOrderId && !paymentStatusParam) {
      const timestamp = parseInt(
        sessionStorage.getItem("wallet_payment_timestamp") || "0",
      );
  ...
  const renderWalletSection = () => (
    <div className="wallet-payment-section">
      ...
      {actionMsg.message && !actionMsg.success && (
        <div
          className={`wallet-message ${actionMsg.success ? "success" : "error"}`}
        >
          {actionMsg.message}
        </div>
      )}
  ...
  const renderCardSection = () => (
    <div className="card-payment-section">
      ...
      {actionMsg.message && (
        <div
          className={`embedded-payment-message ${actionMsg.success ? "success" : "error"}`}
        >
          {actionMsg.message}
        </div>
      )}
            ...
            <div className="paymob-iframe-container">
              <iframe
                ...
                onError={(error) => {
                  ...
                  responseMessageSetter(
                    false,
                    "فشل تحميل نموذج الدفع. يرجى المحاولة مرة أخرى.",
                    setActionMsg,
                  );
                }}
                allow="payment *"
  ...
  return (
    <div className="embedded-payment-wrapper">
      ...
      <div className="payment-action-container">
        {actionMsg.message && (
          <div
            className={`payment-action-message ${actionMsg.success ? "success" : "error"}`}
          >
            {actionMsg.message}
          </div>
        )}
