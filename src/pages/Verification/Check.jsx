      ...
      setTimeout(() => {
        try {
          ...
          navigate("/login", {
            state: {
              message:
                "تم التحقق من بريدك الإلكتروني بنجاح. يرجى تسجيل الدخول للمتابعة.",
            },
          });
        }
      }, 4000);
  ...
  const resendToken = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return setCheckMessage({
          success: false,
          message: data.message || "فشل إعادة الإرسال",
        });
      }

      setCheckMessage({
        success: true,
        message: data.message || "تم إعادة إرسال رابط التحقق",
      });
      setIsResending(false);
  ...
  const getContent = () => {
    if (isLoading) {
      return {
        ...
        subtitle:
          "يرجى الانتظار أثناء التحقق من بريدك الإلكتروني. قد يستغرق ذلك بضع ثوانٍ...",
      ...
      return {
        ...
        subtitle:
          "تم تفعيل بريدك الإلكتروني بالفعل. يمكنك إغلاق هذه النافذة والعودة إلى التطبيق.",
        showResendButton: false,
        showRetryButton: false,
        showDashboardButton: false,
      ...
      return {
        ...
        subtitle:
          "تعذر إرسال رابط التحقق. يرجى التحقق من اتصالك والمحاولة مرة أخرى.",
        showResendButton: true,
        showRetryButton: false,
        showDashboardButton: false,
        showLoginButton: false,
