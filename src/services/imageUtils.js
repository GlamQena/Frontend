export const buildImgSrc = (imagePath, type = "product") => {
  ...
  if (imagePath.includes("uploads")) {
    ...
    return `${process.env.EXPRESS_APP_API_URL || "http://localhost:8080"}/uploads/${formattedPath}`;
  }

  // If it's just a filename
  if (!imagePath.startsWith("/")) {
    return `${process.env.EXPRESS_APP_API_URL || "http://localhost:8080"}/uploads/${imagePath}`;
  }
