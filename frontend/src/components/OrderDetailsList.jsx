...
const XIcon = () => (
  <svg viewBox="0 0 24 24" width="14" height="14">
    <line x1="5" y1="5" x2="19" y2="19" stroke="red" strokeWidth="2.5" strokeLinecap="round" />
    <line x1="19" y1="5" x2="5" y2="19" stroke="red" strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);
const XWhiteIcon = () => (
  <svg viewBox="0 0 24 24" width="14" height="14">
...
function getCompletedStepsBeforeCancel(order, includeReady = false) {
  const ORDER = includeReady
    ? ["pending", "preparing", "ready", "shipping", "delivered"]
    : ["pending", "preparing", "shipping", "delivered"];
  const idx = ORDER.indexOf(normalizeStatus(order.lastStatusBeforeCancel || order.status));
  return idx === -1 ? 0 : idx;
}
...
function ReviewModal({ product, orderId, storeOwnerId, onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || "فشل إرسال التقييم");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <svg width="22" height="21" viewBox="0 0 20 19" fill="none">
            <path d="M3.825 19L5.45 11.975L0 7.25L7.2 6.625L10 0L12.8 6.625L20 7.25L14.55 11.975L16.175 19L10 15.275L3.825 19Z" fill="#E9C349" />
          </svg>
...
// ─── Tracking Stepper ─────────────────────────────────────────────────────────
function TrackingStepper({ order, isCancelled, includeReady }) {
  const STEPS = includeReady ? STORE_TRACKING_STEPS : CLIENT_TRACKING_STEPS;
  const ORDER_ARR = includeReady
    ? ["pending", "preparing", "ready", "shipping", "delivered"]
    : ["pending", "preparing", "shipping", "delivered"];

  const filteredSteps = isCancelled
    ? STEPS.filter((s) => ORDER_ARR.indexOf(s.key) <= getCompletedStepsBeforeCancel(order, includeReady))
    : STEPS;

  return (
    <div className="od-steps">
      {filteredSteps.map((step, i, arr) => {
        const state = isCancelled ? "done" : getStepState(step.key, order.status, includeReady);
        const isLastFiltered = isCancelled && i === arr.length - 1;
        const isLast = !isCancelled && i === STEPS.length - 1;
      ...
      {isCancelled && (
        <div className="od-step od-step--cancelled od-step--last">
          <div className="od-step-indicator">
            <div className="od-step-circle circle--cancelled"><XWhiteIcon /></div>
          </div>
          <div className="od-step-text">
  ...
  return (
    <div className="od-grid">
      <aside className="od-tracking-card">
        ...
        <TrackingStepper order={order} isCancelled={isCancelled} includeReady={false} />
      ...
      <div className="od-card od-shipping-card">
        ...
        <div className="od-info-list">
          <div className="od-info-row">
            ...
            <span className="od-info-val">{shipping.name || order.user_id?.name || "—"}</span>
          </div>
          <div className="od-info-row">
          ...
          <div className="od-info-row od-address-row">
            ...
            <span className="od-info-val od-address-val">
              {shipping.street ? `${shipping.street}، ${shipping.city || ""}، مصر.` : "—"}
            </span>
          </div>
        </div>
      </div>
...
function StoreOrderView({ order, normalizedStatus, isCancelled }) {
  ...
  return (
    <div className="od-grid">
      <aside className="od-tracking-card">
        <h2 className="od-section-title">تتبع الطلب</h2>
        <TrackingStepper order={order} isCancelled={isCancelled} includeReady={true} />
        <div className="od-totals">
          <div className="od-total-row">
            <span className="od-total-label">المجموع الفرعي</span>
            <span className="od-total-val">{order.subtotal_price} ج.م</span>
      ...
      <div className="od-card od-client-info-card">
        <div className="od-card-header">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" fill="#A855F7" />
          </svg>
          <h2 className="od-section-title">معلومات العميل</h2>
        </div>
        <div className="od-client-name">{order.user_id?.name || shipping.name || "—"}</div>
        <div className="od-info-list">
          <div className="od-info-row od-icon-row">
            <span className="od-icon-label">📞</span>
            <span className="od-info-val">{shipping.phone || order.user_id?.phone || "—"}</span>
          </div>
          <div className="od-info-row od-icon-row">
            <span className="od-icon-label">📍</span>
            <span className="od-info-val">
              {shipping.street ? `${shipping.street}، ${shipping.city || ""}` : "—"}
            </span>
          </div>
          <div className="od-info-row od-icon-row">
      ...
      <div className="od-card od-order-info-card">
        <div className="od-card-header">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm-7 14l-5-5 1.41-1.41L12 14.17l7.59-7.59L21 8l-9 9z" fill="#A855F7" />
          </svg>
        ...
        <div className="od-info-list">
          ...
          <div className="od-info-row">
            ...
            <span className="od-info-val od-grand-highlight">{order.total_price} ج</span>
          </div>
        </div>
      </div>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function MyOrderDetails({ historyEndpoint }) {
...
const fetchOrder = async () => {
  try {
    ...
    console.log("ROLE =>", getUserRole()); // ← check this too
    ...
    setError(err.response?.data?.message || "تعذّر تحميل تفاصيل الطلب");
  } finally {
    setLoading(false);
  }
};
        ...
        <div className="od-header-info">
          <div className="od-title-row">
            ...
            <span className={`od-badge ${badge.cls}`}>
              ...
              {normalizedStatus === "cancelled" && <span className="badge-x"><XIcon /></span>}
              {badge.label}
            </span>
          </div>
        ...
        <ReviewModal
          ...
          onSuccess={() => {
            ...
            setRatedProducts((prev) =>
              prev.includes(realProdId.toString()) ? prev : [...prev, realProdId.toString()]
            );
            setReview(null);
          }}
        />
